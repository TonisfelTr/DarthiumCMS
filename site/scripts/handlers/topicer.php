...
if ($session === TRUE) {
    ...
    if (isset($_POST["topic-create-btn"])) {
        if ($user->getUserGroup()->getPermission("topic_create")) {
            ...
            if ($category != "none") {
                ...
                if (\Forum\ForumAgent::GetCategoryParam($category, "public") !== 1 &&
                    (!$user->getUserGroup()->getPermission("category_see_unpublic") || !$user->getUserGroup()->getPermission("category_params_ignore"))) {
                    header("Location: ../../?page=newtopic&res=3np");
                    exit;
                }

                $category = new \Forum\Models\Category($category);
                if (!$category->CanCreateTopic() && !$user->getUserGroup()->getPermission("category_params_ignore")) {
                    header("Location: ../../?page=newtopic&res=3np");
                    exit;
                }
    ...
    if (isset($_POST["topic-edit-btn"])) {
        ...
        if ($user->getId() == $topic->getAuthorId()) {
            if (!$user->getUserGroup()->getPermission("topic_edit")) {
                header("Location: ../../?topic=$id&edit&res=3np");
                exit;
            }
        } else {
            if (!$user->getUserGroup()->getPermission("topic_foreign_edit")){
                header("Location: ../../?topic=$id&edit&res=3np");
                exit;
            }
        }
        if ($topic->getCategoryId() != $_POST["topic-category-select"] && !$user->getUserGroup()->getPermission("topic_manage")){
            header("Location: ../../?topic=$id&edit&res=3np");
            exit;
        }
        if ($topic->getStatus() != $_POST["topic-status"] && !$user->getUserGroup()->getPermission("topic_manage")){
            header("Location: ../../?topic=$id&edit&res=3np");
            exit;
        }
        ...
        if ($category != "none") {
            ...
            if (\Forum\ForumAgent::GetCategoryParam($category, "public") !== 1 &&
                (!$user->getUserGroup()->getPermission("category_see_unpublic"))) {
                header("Location: ../../?topic=$id&edit&res=3np");
                exit;
            }
    ...
    if (isset($_POST["topic-remove-btn"])){
        ...
        if (($user->getId() == $topic->getAuthorId() && $user->getUserGroup()->getPermission("topic_delete")) || $user->getUserGroup()->getPermission("topic_foreign_delete")){
    ...
    if (isset($_POST["comment-delete-btn"])){
        ...
        if (($user->getId() == $comment->getAuthorId() && $user->getUserGroup()->getPermission("comment_delete")) ||
            $user->getUserGroup()->getPermission("comment_foreign_delete")){
    ...
    if (isset($_POST["comment-edit-btn"])){
        ...
        if (($user->getUserGroup()->getPermission("comment_edit") && $comment->getAuthorId() == $user->getId())
            || $user->getUserGroup()->getPermission("comment_foreign_edit")) {
