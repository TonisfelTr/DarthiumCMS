...
Tag::create("{INDEX_PAGE_NAVBAR}")->setProcessingFunction(function () { return BuildManager::includeContentFromTemplate("navbar.html"); });
Tag::create("{INDEX_NAVBAR_BTNS}")->setProcessingFunction(function () {
   return BuildManager::includeContentFromTemplate("nav-btn.html");
});
Tag::create("{END_NAVBAR_BTNS}")->setProcessingFunction(function () {
   return BuildManager::includeContentFromTemplate("nav-btn-end.html");
});
Tag::create("{INDEX_PROFILE_MENU}")->setProcessingFunction(function () { return BuildManager::includeContentFromTemplate((UserAgent::isAuthorized() ? "pam_auth" : "pam_unauth") . ".html"); });
Tag::create("{INDEX_PROFILE_MENU:MAIL_SPAN_CLASS}")->setProcessingFunction(function () use ($user) { return $user === false ? "" : "nav-btn-" . (($user->MessageManager()->getNotReadCount() > 0) ? "new-" : "") . "counter"; });
...
Tag::create("{INDEX_PROFILE_MENU:NOTIF_SPAN_CLASS}")->setProcessingFunction(function () use ($user) { return $user === false ? "" : "nav-btn-" . (($user->Notifications()->getNotificationsUnreadCount()) ? "new-"
        : "") . "counter"; });
...
Tag::create("{INDEX_PROFILE_MENU:USER_NICKNAME}")->setProcessingFunction(function () use ($user) { return $user ? $user->getNickname() : "{%|profile}"; });
...
Tag::create("{INDEX_CATEGORY_LIST}")->setProcessingFunction(function () {
    return BuildManager::includeContentFromTemplate("category_list.html");
});
Tag::create("{INDEX_PAGE_HEADER}")->setProcessingFunction(function () { return BuildManager::includeContentFromTemplate("header.html"); });
Tag::create("{INDEX_PAGE_FOOTER}")->setProcessingFunction(function () { return BuildManager::includeContentFromTemplate("footer.html"); });
Tag::create("{CURRENT_YEAR}")->setProcessingFunction(function () { return date("Y"); });
...
Tag::create("{LAST_SITE_TOPICS}")->setProcessingFunction(function () { return BuildManager::includeContentFromTemplate("last_topic_list.html"); });
Tag::create("{STATISTIC_NOW_AVAILABLE}")->setProcessingFunction(function () { return BuildManager::includeContentFromTemplate("online_list.html"); });
...
Tag::create("{INDEX_CATEGORY_HINT}")->setProcessingFunction(function () {
    return BuildManager::includeContentFromTemplate('category_hint.html');
});
Tag::create("{INDEX_MESSAGE_BOX}")->setProcessingFunction(function () {
    return BuildManager::includeContentFromTemplate('messagebox.html');
});
Tag::create("{INDEX_SEARCHING}")->setProcessingFunction(function () {
    return BuildManager::includeContentFromTemplate('searchpanel.html');
});
...
Tag::create("{INDEX_PAGE_MAIN_BLOCK}", true)
    ...
    switch ($currentRoute->getName()) {
        ...
        case "topic-view":
            return BuildManager::includeFromTemplateAndCompile("news/new.html");
        ...
        case "main-page-paginated":
        {
            return TagAgent::useTemporaryContainer(function () use ($currentRoute) {
                ...
                $newsTemplate             = BuildManager::includeContentFromTemplate("news/preview.html");
                ...
                foreach ($topicsList as $topicData) {
                    ...
                    $topicNameTag->setProcessingFunction(function () use ($topic) {
                        return ($topic->getStatus() == 0
                                ? BuildManager::includeContentFromTemplate('news/lockicon.html')
                    ...
                    $topicStatusIconTag->setProcessingFunction(function () use ($topic) {
                        return ForumAgent::IsExistQuizeInTopic($topic->getId())
                            ? BuildManager::includeContentFromTemplate('news/quizicon.html')
                            : "";
                    });
